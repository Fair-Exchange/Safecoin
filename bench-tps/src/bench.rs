use {
    crate::{
        ...
        send_batch::*,
        spl_convert::FromOtherSafecoin,
    },
    log::*,
    rand::distributions::{Distribution, Uniform},
    rayon::prelude::*,
    safecoin_client::{nonce_utils, rpc_request::MAX_MULTIPLE_ACCOUNTS},
...
fn transfer_with_compute_unit_price_and_padding(
    ...
    let instruction = if let Some(instruction_padding_config) = instruction_padding_config {
        FromOtherSafecoin::from(
            wrap_instruction(
                FromOtherSafecoin::from(instruction_padding_config.program_id),
                FromOtherSafecoin::from(transfer_instruction),
                vec![],
