...
extern uint64_t entrypoint(const uint8_t *input) {
  ...
  case TEST_MAX_INSTRUCTION_ACCOUNTS_EXCEEDED: {
    ...
    SafeAccountMeta *arguments = sol_calloc(accounts_len, sizeof(SafeAccountMeta));
  ...
  case TEST_MAX_ACCOUNT_INFOS_EXCEEDED: {
    ...
    SafeAccountInfo *account_infos = sol_calloc(account_infos_len, sizeof(SafeAccountInfo));
