//! The Safecoin host and client SDK.
//!
//! This is the base library for all off-chain programs that interact with
//! Safecoin or otherwise operate on Safecoin data structures. On-chain programs
//! instead use the [`safecoin-program`] crate, the modules of which are
...
//! provided by `safecoin-program`, see that crate's documentation for an
//! overview.
//!
//! [`safecoin-program`]: https://docs.rs/safecoin-program
//!
//! Many of the modules in this crate are primarily of use to the Safecoin runtime
//! itself. Additional crates provide capabilities built on `safecoin-sdk`, and
//! many programs will need to link to those crates as well, particularly for
//! clients communicating with Safecoin nodes over RPC.
//!
//! Such crates include:
//!
//! - [`safecoin-client`] - For interacting with a Safecoin node via the [JSON-RPC API][json].
//! - [`safecoin-cli-config`] - Loading and saving the Safecoin CLI configuration file.
//! - [`safecoin-clap-utils`] - Routines for setting up the CLI using [`clap`], as
//!   used by the Safecoin CLI. Includes functions for loading all types of
...
// These safecoin_program imports could be *-imported, but that causes a bunch of
// confusing duplication in the docs due to a rustdoc bug. #26211
#[cfg(not(target_os = "solana"))]
pub use safecoin_program::program_stubs;
