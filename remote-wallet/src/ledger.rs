use {
    ...
    safecoin_sdk::derivation_path::DerivationPath,
    std::{fmt, sync::Arc},
};
#[cfg(feature = "hidapi")]
use {
    ...
    safecoin_sdk::{pubkey::Pubkey, signature::Signature},
...
impl RemoteWallet<hidapi::DeviceInfo> for LedgerWallet {
    ...
    fn sign_message(
        ...
        // because it starts with the Domain Specifier b"\xffsafecoin offchain".
    ...
    fn sign_offchain_message(
        ...
        if message.len()
            > safecoin_sdk::offchain_message::v0::OffchainMessage::MAX_LEN_LEDGER
                + safecoin_sdk::offchain_message::v0::OffchainMessage::HEADER_LEN
        {
            return Err(RemoteWalletError::InvalidInput(
