...
//! ```bash
//! safecoin-dos --entrypoint 127.0.0.1:10015 --mode tpu --data-type random
//! ```
//!
//! Example 2: send unique transactions with valid recent blockhash to TPU
//! ```bash
//! safecoin-dos --entrypoint 127.0.0.1:10015 --mode tpu --data-type random
//! safecoin-dos --entrypoint 127.0.0.1:10015 --mode tpu \
...
fn run_dos(
    ...
    if nodes.is_empty() {
        ...
        target = Some((safecoin_sdk::pubkey::new_rand(), params.entrypoint_addr));
    } else {
        info!("************ NODE ***********");
        for node in nodes {
            info!("{:?}", node);
