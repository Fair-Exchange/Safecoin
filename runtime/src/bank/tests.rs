...
fn test_rent_exempt_executable_account() {
    ...
    let mut account = AccountSharedData::new(account_balance, 0, &safecoin_sdk::pubkey::new_rand());
...
fn test_rent_eager_collect_rent_in_partition() {
    ...
    for feature_id in FeatureSet::default().inactive {
        if feature_id != safecoin_sdk::feature_set::set_exempt_rent_epoch_max::id() {
...
fn do_test_bank_update_rewards_determinism() -> u64 {
    ...
    let mut vote_account =
        vote_state::create_account(&vote_id, &safecoin_sdk::pubkey::new_rand(), 0, 100);
    let stake_id1 = safecoin_sdk::pubkey::new_rand();
    let stake_account1 = crate::stakes::tests::create_stake_account(123, &vote_id, &stake_id1);
    let stake_id2 = safecoin_sdk::pubkey::new_rand();
...
#[should_panic(
    expected = "Can't change frozen bank by adding not-existing new precompiled \
               program (EJ7BZ5wk1brwAGwnVmsGauQCRS4btvabkrdn1MpDrUHK). \
               Maybe, inconsistent program activation is detected on snapshot restore?"
)]
...
fn test_bank_executor_cache() {
    ...
    let key5 = safecoin_sdk::pubkey::new_rand();
    ...
    let programdata_key = safecoin_sdk::pubkey::new_rand();
        ...
        ) + std::marker::Send,
{
    ...
    // Set up initial bank
    let mut genesis_config =
        create_genesis_config_with_leader(10, &safecoin_sdk::pubkey::new_rand(), 374_999_998_287_840)
            .genesis_config;
...
fn test_call_precomiled_program() {
    ...
    let instruction =
        safecoin_sdk::secp256k1_instruction::new_secp256k1_instruction(&secp_privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
    ...
    let message_arr = b"hello";
    let instruction =
        safecoin_sdk::ed25519_instruction::new_ed25519_instruction(&privkey, message_arr);
    let tx = Transaction::new_signed_with_payer(
        &[instruction],
        Some(&mint_keypair.pubkey()),
        &[&mint_keypair],
...
fn test_an_empty_instruction_without_program() {
    let (genesis_config, mint_keypair) = create_genesis_config(1);
    let destination = safecoin_sdk::pubkey::new_rand();
...
fn test_invalid_rent_state_changes_fee_payer() {
    ...
    genesis_config.fee_rate_governor = FeeRateGovernor::new(
        safecoin_sdk::fee_calculator::DEFAULT_TARGET_LAMPORTS_PER_SIGNATURE,
        safecoin_sdk::fee_calculator::DEFAULT_TARGET_SIGNATURES_PER_SLOT,
    );
...
fn test_rent_state_incinerator() {
    ...
    for amount in [rent_exempt_minimum - 1, rent_exempt_minimum] {
        bank.transfer(amount, &mint_keypair, &safecoin_sdk::incinerator::id())
            .unwrap();
    }
}
...
fn test_resize_and_rent() {
    ...
    let rent_paying_pubkey = safecoin_sdk::pubkey::new_rand();
...
fn test_cap_accounts_data_allocations_per_transaction() {
    ...
    let (genesis_config, mint_keypair) = create_genesis_config(1_000_000 * LAMPORTS_PER_SAFE);
    let mut bank = Bank::new_for_tests(&genesis_config);
    ...
    assert_eq!(
        ...
        Err(TransactionError::InstructionError(
            ...
            safecoin_sdk::instruction::InstructionError::MaxAccountsDataAllocationsExceeded,
        )),
    );
}
...
fn test_stake_account_consistency_with_rent_epoch_max_feature(
    ...
    solana_logger::setup();
    let (mut genesis_config, _mint_keypair) = create_genesis_config(100 * LAMPORTS_PER_SAFE);
    genesis_config.rent = Rent::default();
    let mut bank = Bank::new_for_tests(&genesis_config);
    let expected_initial_rent_epoch = if rent_epoch_max_enabled_initially {
        bank.activate_feature(&safecoin_sdk::feature_set::set_exempt_rent_epoch_max::id());
