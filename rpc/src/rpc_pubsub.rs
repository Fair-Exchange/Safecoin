...
// `RpcSafePubSubInternal` is actually used by the current PubSub API implementation.
// `RpcSafePubSub` and the corresponding `gen_client` module are preserved
...
pub use internal::RpcSafePubSubInternal;
...
mod internal {
    ...
    #[rpc]
    pub trait RpcSafePubSubInternal {
        // Get notification every time account data is changed
        // Accepts pubkey parameter as base-58 encoded string
...
impl RpcSafePubSubInternal for RpcSafePubSubImpl {
    fn account_subscribe(
        &self,
        pubkey_str: String,
        config: Option<RpcAccountInfoConfig>,
...
mod tests {
    use {
        ...
        serial_test::serial,
        safecoin_account_decoder::{parse_account_data::parse_account_data, UiAccountEncoding},
