...
SAFEANA_RUN_SH_VALIDATOR_ARGS="--snapshot-interval-slots 200" timeout 120 ./scripts/run.sh &
pid=$!
...
while [[ $($solana_cli --url http://localhost:8328 slot --commitment processed) -le $((snapshot_slot + 1)) ]]; do
  sleep 1
  echo "Checking slot"
done
