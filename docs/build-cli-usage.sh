...
cargo build -p safecoin-cli

usage=$(cargo -q run -p safecoin-cli -- -C ~/.foo --help | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')
...
while read -r subcommand rest; do
  ...
  if ((in_subcommands)); then
      section "$(cargo -q run -p safecoin-cli -- help "$subcommand" | sed -e 's|'"$HOME"'|~|g' -e 's/[[:space:]]\+$//')" "####" >> "$out"
  fi
done <<<"$usage">>"$out"
